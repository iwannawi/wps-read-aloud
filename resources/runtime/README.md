...
本目录用于存放不同 CPU 架构和操作系统的原生语音引擎文件。源码、WPS 加载项前端和语音模型共用，只有原生二进制和动态库按目标区分。
...
- x64 银河麒麟 V10 及以上和 x64 UOS V20 安装包共用 “linux-amd64” 运行时资源。
- ARM64 银河麒麟 V10 及以上和 ARM64 UOS V20 安装包共用 “linux-arm64” 运行时资源。
