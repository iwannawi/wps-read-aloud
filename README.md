...
- x64/ARM64 银河麒麟 V10 及以上
- x64/ARM64 UOS V20
- x86/x64 Windows 10/11：WPS Office 2019 或更高版本，推荐 WPS Office 最新稳定版
- x64/ARM64 银河麒麟 V10 及以上、x64/ARM64 UOS V20：WPS Office 2019 或更高版本，推荐最新版 WPS Office for Linux
- 允许安装 WPS JS 加载项
...
当前项目采用同一套源码、多平台打包的交付方式。每次正式出版本都生成五类安装包：x86/x64 Windows 10/11 exe 安装程序、x64 银河麒麟 V10 及以上 deb、ARM64 银河麒麟 V10 及以上 deb、x64 UOS V20 deb、ARM64 UOS V20 deb。各平台共用加载项前端、服务端业务逻辑、文档和模型资源；差异部分集中在原生语音引擎、daemon 二进制、服务启动方式、包名和安装路径。
...
## 项目结构
    ...
    resources/runtime/             按 CPU 架构和操作系统区分的原生语音引擎资源
...
服务只监听 “127.0.0.1:19860”，不访问外网。加载项与服务端通过本机回环地址通信，避免暴露到局域网。安装脚本会尽量只增删本项目自己的 WPS 加载项条目，不覆盖其他已安装加载项；x64/ARM64 银河麒麟 V10 及以上、x64/ARM64 UOS V20 包也包含旧包名的冲突和替换声明，降低升级时的文件归属冲突风险。
...
这些文件会被打入对应安装包。x64/ARM64 银河麒麟 V10 及以上安装包安装到 “/opt/wps-read-aloud-comate”；x64/ARM64 UOS V20 安装包按 UOS 应用目录规范安装到 “/opt/apps/cn.wps-read-aloud-comate/files”；x86/x64 Windows 10/11 安装包安装到用户选择的程序目录。正式构建统一从 “resources/runtime” 读取对应 CPU 架构和操作系统的运行时，不再从旧版 “engines” 目录复制，避免把目标环境不需要的库和废弃语音引擎带入安装包。
...
如果某个目标缺少对应 CPU 架构和操作系统的 Sherpa-onnx 运行文件、模型资源或 daemon 二进制，脚本会停止并提示缺失路径，避免生成不可用安装包。

正式发布版本必须一次性生成五类安装包，并通过发布前检查：

    python packaging/verify_release_artifacts.py

该检查会确认五个安装包、五个 SHA256 文件和 “CHECKSUMS.txt” 完全一致，同时检查安装包内没有混入目标环境不需要的资源，例如 x86/x64 Windows 10/11 包不包含 systemd 文件，x64/ARM64 银河麒麟 V10 及以上、x64/ARM64 UOS V20 包不包含 Windows exe，所有包都不包含 Piper 或 eSpeak NG 等已弃用资源。
...
- x64/ARM64 UOS V20：安装程序文件和配置文件到 “/opt/apps/cn.wps-read-aloud-comate/files”
- 安装并启动 “wps-tts.service”
- 为已有普通用户注册 WPS JS 加载项
- 写入安装日志 “/var/log/wps-read-aloud-install.log”
- 安装第三方组件许可证和交付说明；x64/ARM64 银河麒麟 V10 及以上路径为 “/usr/share/doc/wps-read-aloud-comate”，x64/ARM64 UOS V20 路径为 “/opt/apps/cn.wps-read-aloud-comate/files/doc”
