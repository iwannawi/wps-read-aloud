...
- “文档朗读”选项卡 6 个按钮图标回退到上一版稳定图标文件：开始朗读、停止朗读、朗读方式、朗读语速、状态检查、关于朗读。
...
- 软件包文件名统一为小写 `wps-read-aloud-xc_1.0.22_arm64.deb`，与 Debian 内部包名 `wps-read-aloud-xc` 保持一致。
...
```bash
sudo dpkg -i wps-read-aloud-xc_1.0.22_arm64.deb
```
