...
| --- | --- | --- |
| x86/x64 Windows 10/11 | WPS Office 2019 或更高版本 | wps-read-aloud-comate_1.1.13_windows.exe |
| x64 银河麒麟 V10 及以上 | WPS Office 2019 for Linux 或更高版本 | wps-read-aloud-comate_1.1.13_amd64.deb |
| ARM64 银河麒麟 V10 及以上 | WPS Office 2019 for Linux 或更高版本 | wps-read-aloud-comate_1.1.13_arm64.deb |
| x64 UOS V20 | WPS Office 2019 for Linux 或更高版本 | cn.wps-read-aloud-comate_1.1.13_amd64.deb |
| ARM64 UOS V20 | WPS Office 2019 for Linux 或更高版本 | cn.wps-read-aloud-comate_1.1.13_arm64.deb |

## 安装验收

| CPU 架构 + 操作系统 | 操作 |
| --- | --- |
| x86/x64 Windows 10/11 | 运行 wps-read-aloud-comate_1.1.13_windows.exe |
| x64 银河麒麟 V10 及以上 | sudo dpkg -i wps-read-aloud-comate_1.1.13_amd64.deb |
| ARM64 银河麒麟 V10 及以上 | sudo dpkg -i wps-read-aloud-comate_1.1.13_arm64.deb |
| x64 UOS V20 | sudo dpkg -i cn.wps-read-aloud-comate_1.1.13_amd64.deb |
| ARM64 UOS V20 | sudo dpkg -i cn.wps-read-aloud-comate_1.1.13_arm64.deb |
...
| 服务管理 | 按需启动，停止朗读后退出，不写入 Run 自启动 | systemd 管理 wps-read-aloud-comate.service | systemd 管理 wps-read-aloud-comate.service |
...
- 安装完成后，任务管理器中不应长期存在 wps-tts-daemon.exe。
- 打开 WPS 但未使用朗读功能时，任务管理器中不应长期存在 wps-tts-daemon.exe。
- 点击“开始朗读”后，wps-tts-daemon.exe 可按需启动。
- 点击“停止朗读”或朗读完成后，wps-tts-daemon.exe 应退出；如 WPS 异常关闭，服务空闲后也应自动退出。
- 开始菜单“WPS 文档朗读助手”目录下应有卸载入口。
- 系统“应用和功能”中应显示“WPS 文档朗读助手”，并可执行卸载。
- 卸载后应清理安装目录、WPS 加载项条目、旧版 Run 自启动项、旧计划任务、开始菜单入口和卸载注册表项。
...
示例文本：
    ...
    这是 WPS 2026 read aloud test，版本是 v1.1.13。
