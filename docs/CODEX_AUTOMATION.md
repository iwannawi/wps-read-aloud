...
1. 更新版本号和发布说明。
2. 重新构建 x86/x64 Windows 10/11、x64/ARM64 银河麒麟 V10 及以上、x64/ARM64 UOS V20 五类安装包。
3. 拆包检查各目标的安装路径、LF 换行、权限、符号链接、图标和可疑文件。
4. 计算 SHA256。
5. 提交源码变更。
6. 创建并推送标签，格式为 “v版本号-发布日期”。
7. 保持 “.exe”、“.deb” 和对应校验文件不进入普通 Git 提交。
...
- “.exe”、“.deb”、语音模型、离线引擎、工具链和构建缓存不进入普通 Git。
