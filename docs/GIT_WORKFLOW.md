...
```powershell
.\scripts\push_github.ps1 -Tag v1.0.23-20260518
.\scripts\publish_github_release.ps1 -Version 1.0.23 -ReleaseDate 20260518
```
...
从 `1.0.23` 开始，服务版本号由 `/opt/wps-read-aloud/version.json` 提供。仅修改加载项前端、图标、弹窗样式或说明文件时，不需要重新编译 Go 服务；打包脚本会从当前 `dist/wps-tts-daemon` 或最近一个已生成的 `.deb` 中复用 daemon 二进制。

当前交付包命名示例：

```text
wps-read-aloud-xc_1.0.23_arm64.deb
```
